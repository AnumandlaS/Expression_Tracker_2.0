  ...
  return (
    ...
    <div className="home-container">
      ...
      <main className="main-content" style={{ marginTop: '70px' }}>
        <div className="home-buttons">
          <button className="btn btn-primary" onClick={() => window.location.href = '/login'}>Play the Game</button>
          <button className="btn btn-secondary" onClick={() => window.location.href = '/analysis'}>Get Analysis</button>
        </div>
      </main>
      {/* <footer  className="footer " style={{
        backgroundColor: 'rgba(255, 255, 255, 0.7)', // Pale white with 70% opacity
        boxShadow: 'none',
      }} >© 2024 Expression Tracker. All rights reserved.</footer> */}
    </div>
    </div>
  );
};
