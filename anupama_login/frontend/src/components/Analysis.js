...
const Analysis = () => {
  ...
  const fetchSessions = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/sessions`);// send get request to backend that is server for getting the session existing id
  ...
  const handleSessionClick = async (sessionId) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 404) {
        ...
        try {
          console.log(3);
          const mediaResponse = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/sessions/media/${sessionId}`);
          ...
          if (media.length === 0) {
           ...
           const analysisResponse = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/sessions/analyze/${sessionId}`, { images: media });
