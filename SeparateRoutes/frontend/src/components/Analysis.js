...
const Analysis = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/sessions`
      );
  ...
  const handleSessionClick = async (sessionId) => {
    ...
    try {
      ...
      console.log(2);
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/admin/sessions/analysis/${sessionId}`
      );
      console.log("supppp");
      ...
      if (error.response && error.response.status === 404) {
        console.log(
          "No analysis found. Sending images to Hugging Face model..."
        );
        try {
          console.log(3);
          ...
          const analysisResponse = await axios.post(
            `${process.env.REACT_APP_BACKEND_URL}/admin/sessions/analyze/${sessionId}`
          );
          ...
          if (analysisResponse.data && analysisResponse.data.analysisResults) {
            ...
            console.error(
              "Analysis response did not contain results:",
