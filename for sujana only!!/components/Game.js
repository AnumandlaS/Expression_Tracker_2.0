...
// //         console.log("Image captured. Session ID: " + newSessionId);
...
const Game = () => {
  ...
  const captureImage = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      console.log("Image captured. Session ID: " + newSessionId + " | Session Name: " + sessionName);
    
      // Send the image, session ID, and session name to the server
      axios.post('http://localhost:5000/uploads', formData)
        .then((response) => console.log('Image uploaded with session details:', response.data))
        .catch((error) => console.error('Error uploading image with session details:', error));
    });
